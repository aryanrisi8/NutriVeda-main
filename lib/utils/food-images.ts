...
export async function generateFoodImageUrl(foodName: string): Promise<string> {
  try {
    ...
    const unsplashUrl = `https://api.unsplash.com/search/photos?query=${encodeURIComponent(foodName + ' food')}&per_page=1&orientation=square`
  ...
  try {
    const foodishResponse = await fetch('https://foodish-api.herokuapp.com/api/')
  ...
  return `https://source.unsplash.com/400x400/?${encodeURIComponent(foodName)},food`
}
...
export function getEnhancedFoodImageUrl(foodName: string): string {
  ...
  const mappedQuery = indianFoodImageMap[lowerName] || `${foodName.replace(/\s+/g, '-')}-food`
